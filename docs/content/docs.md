...
Rclone supports preserving metadata on files (not directories) when
...
in their docs. (Eg [local](/local/#metadata), [s3](/s3/#metadata))
...
The consequence of 2 is that you should can copy an S3 object with
metadata to Azureblob (say) and have the metadata appear on the
Azureblob object also.
